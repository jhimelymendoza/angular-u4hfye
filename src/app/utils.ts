...
export default function createNumberMaskMy({
  prefix = dollarSign,
  suffix = emptyString,
  ...
  const addThousandsSeparator = (n, thousandsSeparatorSymbol?: string) => {
    return n.replace(/\B(?=(\d{3})+(?!\d))/g, thousandsSeparatorSymbol);
  };
  debugger;

  return (rawValue = emptyString) => {
